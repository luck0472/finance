...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < total_cost:
            return apology("You don't have enogh cash")
...
def register():
    ...
    if request.method == "POST":
        # credentials
        if not request.form.get("username"):
            return apology("Username is requered", 400)
        elif not request.form.get("password"):
            return apology("Password is requered", 400)
        ...
        # search for the newly register user
        rows = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))
